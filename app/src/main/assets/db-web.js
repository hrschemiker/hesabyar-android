'use strict';
const initSqlJs = (typeof window!=='undefined' ? window.initSqlJs : require('sql.js'));
...
let persist = null;
async function init(persistence) {
  persist = persistence;
  SQL = await initSqlJs({ locateFile: function (f) { return f; } });
  const bytes = persist.load();
  if (bytes && bytes.length) db = new SQL.Database(bytes); else db = new SQL.Database();
...
function scheduleSave() {
  if (saveTimer) return;
  saveTimer = setTimeout(function () { saveTimer = null; save(); }, 0);
}
function save() {
  if (!db || !persist) return;
  try { persist.save(db.export()); } catch (e) { /* ignore */ }
}
